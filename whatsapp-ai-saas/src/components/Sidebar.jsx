...
const Sidebar = ({ instances, activeId, onSelect, onAdd, onRemove, onUpdate, currentPath }) => {
    ...
    // Sélection sans repli : `|| []` produirait un tableau neuf à chaque rendu,
    // ce qui invaliderait les useMemo ci-dessous en permanence. Le cas indéfini
    // est traité dans les mémos, dont la dépendance reste ainsi stable.
    const savedMenuOrder = useAppStore(state => state.appSettings?.mainMenuOrder);
    const hiddenMenuItems = useAppStore(state => state.appSettings?.hiddenMenuItems);
    ...
    // Liste dérivée, et non copiée dans un état local. `useState(orderedMenu)`
    // figeait la valeur du premier rendu : ni les réglages restaurés depuis
    // IndexedDB — qui arrivent de façon asynchrone — ni le masquage d'un onglet
    // ne se reflétaient avant un redémarrage.
    const menuItems = useMemo(() => {
        const hidden = hiddenMenuItems || [];
        return hidden.length === 0
            ? orderedMenu
            : orderedMenu.filter(item => !hidden.includes(item.id));
    }, [orderedMenu, hiddenMenuItems]);
    ...
    const handleNavDragEnd = (event) => {
        ...
        // Le réordonnancement s'applique à la liste COMPLÈTE, pas à la liste
        // affichée : sans cela, déplacer un onglet visible ferait perdre leur
        // position aux onglets masqués, qui réapparaîtraient en fin de liste.
        const ids = orderedMenu.map(i => i.id);
        const oldIndex = ids.indexOf(active.id);
        const newIndex = ids.indexOf(over.id);
        if (oldIndex === -1 || newIndex === -1) return;

        updateSettings({ mainMenuOrder: arrayMove(ids, oldIndex, newIndex) });
    };

    ...
    return (
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col h-full">
            <div className="flex items-center gap-3 mb-4 text-primary">
                ...
                <h3 className="text-lg font-semibold text-gray-900">{t(language, 'profitCalculator')}</h3>
            </div>

            <div className="space-y-4 mb-6 flex-1">
                <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">{t(language, 'costExpense')}</label>
                    <input
                        type="number"
                        value={cost}
                ...
                <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">{t(language, 'sellingPrice')}</label>
                    <input
                        type="number"
                        value={revenue}
            ...
            <div className="bg-gray-50 p-4 rounded-lg flex items-center justify-between border border-gray-100">
                <div>
                    <p className="text-xs text-gray-500">{t(language, 'netProfit')}</p>
                    <p className={`text-lg font-bold ${parseFloat(profit) >= 0 ? 'text-green-600' : 'text-red-500'}`}>${profit}</p>
                </div>
                <div className="text-right">
                    <p className="text-xs text-gray-500">{t(language, 'grossMargin')}</p>
    ...
    return (
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col h-full">
            <div className="flex items-center gap-3 mb-4 text-blue-500">
                ...
                <h3 className="text-lg font-semibold text-gray-900">{t(language, 'waFormatter')}</h3>
            </div>

            <p className="text-xs text-gray-500 mb-4">{t(language, 'waFormatterDesc')}</p>
            ...
            <div className="flex gap-3">
                <div className="flex-1 bg-gray-50 p-2 text-sm rounded border border-gray-200 truncate select-all">
                    {applyFormat() || <span className="text-gray-400">{t(language, 'resultHere')}</span>}
                </div>
                <button
                    onClick={handleCopy}
                    disabled={!text}
    ...
    return (
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col h-full lg:col-span-2">
            <div className="flex items-center gap-3 mb-4 text-purple-500">
                ...
                <h3 className="text-lg font-semibold text-gray-900">{t(language, 'discountPromo')}</h3>
            </div>

            <div className="flex flex-col md:flex-row gap-6 mb-6">
                <div className="flex-1 space-y-4">
                    <div>
                        <label className="block text-xs font-medium text-gray-500 mb-1">{t(language, 'originalPrice')}</label>
                        <input
                            type="number"
                            value={price}
                    ...
                    <div>
                        <label className="block text-xs font-medium text-gray-500 mb-1">{t(language, 'discountPercent')}</label>
                        <input
                            type="number"
                            value={discount}
                ...
                <div className="flex-1 flex flex-col justify-center gap-4 bg-purple-50 p-6 rounded-xl border border-purple-100">
                    <div className="flex justify-between items-end border-b border-purple-200 pb-2">
                        <span className="text-sm font-medium text-purple-800">{t(language, 'finalPrice')}</span>
                        <span className="text-2xl font-bold text-purple-900">${final}</span>
                    </div>
                    <div className="flex justify-between items-end">
                        <span className="text-xs font-medium text-purple-600">{t(language, 'totalSavings')}</span>
...
const ToolsBox = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div className="mb-8">
                <h1 className="text-2xl font-bold text-gray-900">{t(language, 'utilityTools')}</h1>
                <p className="text-gray-500 text-sm mt-1">{t(language, 'utilityToolsDesc')}</p>
            </div>

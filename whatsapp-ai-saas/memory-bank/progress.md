...
- [x] **Dette technique — Lot 4 (C5) v1.48.1 (2026-08-29)** :
  ...
  - **Correctif latent découvert par le lot** : `dompurify` (correctif C2 v1.47.2) n'était pas déclaré dans `package.json` (installé sans --save) — tout `npm ci` propre cassait le build. Déclaré `^3.4.14` ; scan anti-fantômes sur le périmètre bundlé (src/ + electron/) : plus aucune dépendance non déclarée.

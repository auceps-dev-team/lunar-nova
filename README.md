...
<p align="center">
  <a href="https://github.com/auceps-dev-team/lunar-nova"><img src="https://img.shields.io/badge/version-1.48.3-blue.svg" alt="Version 1.48.3" /></a>
...
**État réel de la couverture (v1.48.3).** **255 tests** répartis dans **28 suites** couvrent aujourd'hui les zones qui cassent en production : chiffrement des secrets (`secretStore`, 13 tests — dont l'observabilité de l'échec de déchiffrement), extraction JSON des réponses LLM (`llmJson`, 18), numérotation (`phoneRules` 20 + `phoneFormat` 13), migrations SQLite (5), parité i18n des 4 langues (3), CLI entrant (`cliInbound`, 13) et flux CLI/MCP bout-en-bout (`cliMcpFlow`, 6), routage agentique (`agentFallback`, 6 — les tests d'intégration réels s'ignorent proprement sans clé API ; `agentFallbackStrategies`, 6 — mécanique des stratégies CLI/API/MCP et repli, remontée du stderr d'échec et trust workspace Gemini, déterministe et sans clé), CRM (`crmService`, 6), parseurs de scrapers (14), comparaison de versions de l'updater (14), etc. Le gros du code reste non testé, et c'est la contribution la plus utile qu'on puisse apporter au projet. Les zones encore à couvrir :
